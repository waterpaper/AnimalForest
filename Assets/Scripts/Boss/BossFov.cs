...
public class BossFov : MonoBehaviour
{
    ...
    public GameObject tracePlayer()
    {
        ...
        Collider[] colls = Physics.OverlapSphere(bossTr.position, viewAngle, 1 << playerLayer);

        //배열의 개수가 1 이상일때 오브젝트 범위 안에 있다고 판단하고 검사후 가장 가까운 플레이어 오브젝트를 반환합니다.
        for (int i = 0; i < colls.Length; i++)
        {
            //적 캐릭터와 주인공 사이의 방향 벡터를 계산
            Vector3 dir = (colls[i].gameObject.transform.position - bossTr.position).normalized;

            //적 캐릭터의 시야각에 들어왔는지 판단
            if (Vector3.Angle(bossTr.forward, dir) < viewAngle * 0.5f)
            {
                //플레이어와 적 캐릭터 간의 거리를 계산
                float dist = Vector3.Distance(colls[i].gameObject.transform.position, bossTr.position);
    ...
    public bool isViewPlayer(Transform playerTr)
    {
        ...
        if (Physics.Raycast(bossTr.position, dir, out hit, detectRange, layerMask))
        {
